...
def process_DGIdb():
    ...
    for line in inputh:
        ...
        line_a = line.split("\t")
        if (
            re.search("Possible suggestions:", line_a[0]) == None
            and re.search("Unmatched search term:", line_a[0]) == None
            and re.search("gene_name", line_a[0]) == None
        ):
...
def process_sabdab():
    ...
    sabdab_targets = {
        ...
        "APP Abeta 1-40/42;TFRC": "APP;TFRC",
        "AR-C124910XX": "",
        "CALCA&CALCB": "CALCA;CALCB",
        "Canine NGFB": "",
        "CD41_CD61": "CD41;CD61",
        "CEACAM5&CD3E;CD3E": "CEACAM5;CD3E",
        "Dabigatran": "",
        "DNA/H1 Complex": "",
        "ERBB2 (Domain II);ERBB2 (Domain IV)": "ERBB2",
        "FN extra domain B": "FN1",
        "FZD Family": "FZD1;FZD2;FZD3;FZD4;FZD5;FZD6;FZD7;FZD8;FZD9;FZD10",
        "Ganglioside GD2": "B4GALNT1",
        "Heat Shock Protein 90 Homolog": "HSP90",
        "HHV gB AD-1": "",
        "HIV-1 gp120": "",
        "HIV-1 gp120 CD4bs": "",
        "HIV-1 gp120 V3": "",
        "Idiotope of anti-NeuGc-ganliosides": "",
        "IGF1&IGF2": "IGF1;IGF2",
        "IL31 (Canine)": "",
        "IL31 (Feline)": "",
        "Influenza A HA": "",
        "Influenza A HA2": "",
        "Influenza B Virus": "",
        "ITGA2B_ITGB3": "ITGA2B;ITGB3",
        "ITGA4_ITGB7": "ITGA4;ITGB7",
        "ITGA4_ITGB7&ITGAE_ITGB7": "ITGA4;ITGAE;ITGB7",
        "ITGAV_ITGB3": "ITGAV;ITGB3",
        "MRSV envelope protein": "",
        "NGFB (Canine)": "",
        "Non-Binding": "",
        "NOTCH2&3": "NOTCH2;NOTCH3",
        "PcrV type III secretion system": "",
        "PcrV type III secretion system;Polysaccharide synthesis locus (Pseudomonas)": "",
        "PDCD1 (Canine)": "",
        "Phosphatidylserine": "PTDSS1",
        "pro-GDF-8": "MSTN",
...
def process_moalmanac():
    ...
    def process_alterations(selection):
        ...
        for ele in list(set(selection["feature_type"].tolist())):
            alterations.append(
                ele
                + " ("
                + ",".join(list(set(selection[selection["feature_type"] == ele]["protein_change"].dropna().tolist())))
                + ")"
            )
        return "; ".join(alterations).replace(" ()", "")
    ...
    inputf = inputf[
        (
            (inputf["drug"].str.contains("\+") == False)
            & (
                inputf["predictive_implication"].isin(
                    ["FDA-Approved", "Guideline", "Clinical trial", "Clinical evidence"]
                )
            )
        )
        | ((inputf["drug"].str.contains("\+")) & (inputf["predictive_implication"].isin(["FDA-Approved", "Guideline"])))
    ]
    ...
    for index, row in subset_add.iterrows():
        selection = inputf[
            ((inputf["gene"] == row["gene"]) | (inputf["gene1"] == row["gene"]) | (inputf["gene2"] == row["gene"]))
            & (inputf["drug"] == row["drug"])
        ]
...
def process_GDSC():
    ...
    GDSC_int = "GDSC_intermediate.tsv"
    GDSC = "GDSC.tsv"
    ...
    def label_sens(row):
        if row["feature_delta_mean_ic50"] < 0:
            return "sensitivity"
        else:
            return "resistance"
        ...
        if re.search("_mut", row["feature_name"]) != None:
            ...
            if re.search("loss|gain", field_a[3]) != None:
                        ...
                        [
                            ...
                            for sublist in [
                                ...
                                for y in [
                                    str(x)
                                    for x in gfeatures[
                                        (gfeatures["genetic_feature"].isin(pan_list)) & (gfeatures["is_mutated"] == 1)
                                    ]["genes_in_segment"]
                                ]
                            ]
                            for item in sublist
                ...
                if row["feature_name"] == "gain.cnaPANCAN393..FOXA1.NKX2.1.PSMA6.":
                    field_a[field_a.index("1") - 1 : field_a.index("1") + 1] = [
                        "-".join(field_a[field_a.index("1") - 1 : field_a.index("1") + 1])
                    ]
                    ...
                    [
                        ...
                        for sublist in [
                            ...
                            for y in [
                                str(x)
                                for x in gfeatures[
                                    (gfeatures["genetic_feature"] == row["feature_name"].split(".")[1])
                                    & (gfeatures["is_mutated"] == 1)
                                ]["genes_in_segment"]
                            ]
                        ]
                        for item in sublist
            ...
            while "UNCLASSIFIED" in field_a:
                field_a.remove("UNCLASSIFIED")
            return ",".join(field_a)

    def obtain_alt(row):
        if re.search("loss.", row["feature_name"]) != None:
            return "deletion"
        elif re.search("gain.", row["feature_name"]) != None:
            return "amplification"
        elif re.search("_mut", row["feature_name"]) != None:
            return "mutation"

    gfeatures = pd.read_csv(
        pro_dir + "GDSC_features.tsv",
        sep="\t",
        header=0,
        dtype={
            "cell_line_name": str,
            "cosmic_sample_id": int,
            "gdsc_desc1": str,
            "gdsc_desc2": str,
            "tcga_desc": str,
            "genetic_feature": str,
            "is_mutated": int,
            "recurrent_gain_loss": str,
            "genes_in_segment": str,
        },
    )

    filei = pd.read_excel(
        dwn_dir + ANOVA_dwn,
        sheet_name="PANCANCER_ANOVA",
        header=0,
        dtype={
            "drug_name": str,
            "drug_id": int,
            ...
            "log_ic50_mean_pos": float,
            "log_ic50_mean_neg": float,
            "log_max_conc_tested": float,
            "feature_ic50_t_pval": float,
            "feature_delta_mean_ic50": float,
    ...
    filei["gene"] = filei["gene"].apply(lambda x: x.split(","))
    filei = filei.explode("gene")
    filei = filei.reset_index()

    filei.rename(columns={"gene": "gene_name"}, inplace=True)

    filei.to_csv(pro_dir + GDSC_int, sep="\t", index=False, header=True)

    fileo = pd.DataFrame(columns=["gene_name", "drug_name", "source", "response", "alteration"])
    source = "GDSC"

    for index, row in filei[["drug_name", "gene_name"]].drop_duplicates().iterrows():
        selection = filei[(filei["drug_name"] == row["drug_name"]) & (filei["gene_name"] == row["gene_name"])]

        if len(selection) > 0:
            response = []
            (alteration_sen, alteration_res) = ("", "")

            if len(selection.loc[selection["response"] == "sensitivity"]) > 0:
                response.append("sensitivity")
                alteration_sen = "; ".join(
                    list(set(selection.loc[selection["response"] == "sensitivity"]["alteration"].tolist()))
                )
            if len(selection.loc[selection["response"] == "resistance"]) > 0:
                response.append("resistance")
                alteration_res = "; ".join(
                    list(set(selection.loc[selection["response"] == "resistance"]["alteration"].tolist()))
                )
    ...
    fileo.to_csv(pro_dir + GDSC, index=False, sep="\t", header=True)
...
def process_FDA():
    ...
    listdir = os.listdir(dwn_dir)
    for efile in listdir:
        if re.search("^drugsatfda", efile) != None:
            fda_dwn = efile
    ...
    for index, row in df_dict["products"].iterrows():
        ...
        for comp in compound:
            status = df_dict["markstatlook"][
                df_dict["markstatlook"]["MarketingStatusID"]
                == df_dict["markstat"][df_dict["markstat"]["ApplNo"] == applno]["MarketingStatusID"].tolist()[0]
            ]["MarketingStatusDescription"].tolist()
    ...
    for comp in fda_drug_list:
        if "Prescription" in fda_drug_list[comp][0] or "Over-the-counter" in fda_drug_list[comp][0]:
            fda_drug_list[comp][0] = "Approved"
        elif "Discontinued" in fda_drug_list[comp][0]:
            fda_drug_list[comp][0] = "Withdrawn"
        else:
            fda_drug_list[comp][0] = ""
    ...
    fda_status = pd.concat(
        [fda_status, pd.DataFrame(fda_status["data"].to_list(), columns=["status", "ApplNo"])], axis=1
    )
                    ...
                    with z.open(filename) as f:
                        ...
                        for e in data["results"]:
                            if "indications_and_usage" in e.keys() and "application_number" in e["openfda"].keys():
                                fileo.write(
                                    "\t".join([e["indications_and_usage"][0], e["openfda"]["application_number"][0]])
                                    + "\n"
                                )
...
def process_EMA():
    ...
    selection = filei.loc[filei["Category"] == "Human"][
        [
            "Medicine name",
            "Therapeutic area",
...
def process_cgc_oncovar():
    ...
    output.write(
        "\t".join(["gene", "cancer_type", "P_value", "FDR", "OncoVar_Score", "Consensus_Score", "Driver_Level"]) + "\n"
    )
                ...
                output.write(
                    "\t".join(
                        [
                            ...
                            str(row["P_value"]),
                            str(row["FDR"]),
                            str(row["OncoVar_Score"]),
                            str(row["Consensus_Score"]),
                            str(row["Driver Level"]),
                        ]
                    )
                    + "\n"
                )
    ...
    for gene in genes:
        categories = [
            cgc.loc[cgc["Gene Symbol"] == gene]["Role in Cancer"],
            oncovar.loc[oncovar["Gene_symbol"] == gene]["OncoKB"],
            oncovar.loc[oncovar["Gene_symbol"] == gene]["2020Rule"],
            oncovar.loc[oncovar["Gene_symbol"] == gene]["CTAT"],
            oncovar.loc[oncovar["Gene_symbol"] == gene]["Oncogene"],
            oncovar.loc[oncovar["Gene_symbol"] == gene]["TSgene"],
        ]
            ...
            if not cat.empty:
                ...
                if isinstance(value, str):
                    ...
                    for subcat in subcats:
                        if subcat not in list_cat:
                            list_cat.append(subcat.upper())
...
def process_KEGG_ind():
    ...
    if not os.path.exists(pro_dir_KEGG):
        os.makedirs(pro_dir_KEGG)

    if not os.path.exists(pro_dir_KEGG + "/" + "components"):
        os.makedirs(pro_dir_KEGG + "/" + "components")
    if not os.path.exists(pro_dir_KEGG + "/" + "relations"):
        os.makedirs(pro_dir_KEGG + "/" + "relations")
    if not os.path.exists(pro_dir_KEGG + "/" + "subpaths"):
        os.makedirs(pro_dir_KEGG + "/" + "subpaths")
    if not os.path.exists(pro_dir_KEGG + "/" + "dictionaries"):
        os.makedirs(pro_dir_KEGG + "/" + "dictionaries")
    ...
    def retrieve_symbol(gene):
        if gene not in gene_symbol_dic.keys():
            ...
            gene_symbol = gene_symbol_dic[gene]
    ...
    def nodes_ids_and_symbols():
        for i in progressbar.progressbar(range(len(paths))):
            relations = pd.read_csv(
                pro_dir_KEGG + "relations/" + paths[i] + ".relations.tsv", sep="\t", low_memory=False, header=None
            )
            relations.columns = ["entry1", "entry2", "type", "name", "value"]
            components = pd.read_csv(
                pro_dir_KEGG + "components/" + paths[i] + ".components.tsv", sep="\t", low_memory=False, header=None
            )
    ...
    def translate_codes_to_symbols():
        for i in progressbar.progressbar(range(len(paths))):
            relations = pd.read_csv(
                pro_dir_KEGG + "relations/" + paths[i] + ".elemcode.tsv", sep="\t", low_memory=False, header=None
            )
            relations.columns = ["entry1", "entry2", "type", "name", "value"]
    ...
    def obtain_subroutes():
        ...
        for i in progressbar.progressbar(range(len(paths))):
            ...
            dictionary = pd.read_csv(
                pro_dir_KEGG + "dictionaries/" + paths[i] + ".codes.tsv", sep="\t", low_memory=False, header=0
            )
            ...
            for path in all_paths_dedup:
                ...
                for pair in list(more_itertools.windowed(path, n=2, step=1)):
                    associations = [
                        ...
                        for sublist in [
                            x.split(",")
                            for x in filei.loc[(filei["entry1"] == pair[0]) & (filei["entry2"] == pair[1])][
                                "name"
                            ].tolist()
                        ]
                        for item in sublist
                    ...
                    if len(associations) > 0:  # valid: association is in relations file
                        if len(set(associations) & set(["indirect effect"])) == 0:  # valid: asociation is not indirect
                            if (
                                str(dictionary.loc[dictionary["node"] == pair[0]]["symbol"].tolist()[0]) != "nan"
                                and str(dictionary.loc[dictionary["node"] == pair[1]]["symbol"].tolist()[0]) != "nan"
                            ):  # valid: node has gene symbol attached
                                # update source node
                                genes = []
                                for gene in (
                                    dictionary.loc[dictionary["node"] == pair[0]]["symbol"].tolist()[0].split(":")
                                ):
                                    ...
                                    roles = ":".join(
                                        list(
                                            set(
                                                [
                                                    x
                                                    for x in generole.loc[generole["gene"] == gene]
                                                    .values.flatten()
                                                    .tolist()[1:]
                                                    if str(x) != "nan"
                                                ]
                                    ...
                                    if (
                                        (
                                            roles in ["ONC", ""]
                                            and len(set(associations) & set(["activation", "expression"])) > 0
                                        )
                                        or (
                                            roles == "TSG"
                                            and len(set(associations) & set(["inhibition", "repression"])) > 0
                                        )
                                        or (roles not in ["ONC", "", "TSG"])
                                        or (
                                            len(
                                                set(["activation", "expression", "inhibition", "repression"])
                                                & set(associations)
                                            )
                                            == 0
                                        )
                                ...
                                if gene_names != "":  # association only saved if genes in node
                                    if len(path_it) == 0:
                                        ...
                                        path_it[-1] = gene_names

                                    # report destination node
                                    genes = []
                                    for gene in (
                                        dictionary.loc[dictionary["node"] == pair[1]]["symbol"].tolist()[0].split(":")
                                    ):
                                        genes.append(gene)
                                    gene_names = ":".join(genes)
            ...
            all_paths_checked_dedup = [
                list(a) for a in (all_paths_checked for all_paths_checked, _ in itertools.groupby(all_paths_checked))
            ]
            ...
            for path in all_paths_checked_dedup:
                ...
                for path_search in all_paths_checked_dedup:
                    ...
                    if any(path == path_search[i : i + len(path)] for i in range(len(path_search))) and len(
                        path
                    ) != len(path_search):
                        found = True
                        break
                ...
                for idx, col in enumerate(line_a[1:]):
                    for gene in col.split(":"):
                        ...
                        if gene in upstream_genes.keys():
                            upstream_genes[gene] = upstream_genes[gene] + [
                                x for ele in line_a[: idx + 1] for x in ele.split(":")
                            ]
                        else:
...
def process_gene_names():
    ...
    moalmanac_file = "moalmanac.tsv"
    GDSC = "GDSC.tsv"
    drugbank_file = "DrugBank.tsv"

    files = [dgidb, sabdab_file, moalmanac_file, GDSC, drugbank_file]
    ...
    for f in files:
        ...
        for line in inputf:
            line = line.rstrip("\n")
            line_a = line.split("\t")
            if "gene_name" in line_a:
                gene_index = line_a.index("gene_name")
            if f == GDSC:
                genes = genes + line_a[gene_index].split(",")
            elif f == sabdab_file:
                genes = genes + line_a[gene_index].split(";")
            else:
                genes.append(line_a[gene_index])
        inputf.close()
...
def process_civic():
    ...
    civic_select = civic.loc[
        (civic["status"] == "ACCEPTED")
        & (civic["evidence_direction"] == "SUPPORTS")
        & (civic["clinical_significance"].isin(["RESISTANCE", "SENSITIVITYRESPONSE"]))
        & (civic["evidenceLevel"].isin(["A"]))
...
def process_oncoKB():
    ...
    oncokb_select = oncokb.loc[
        (oncokb["Level"].isin(["1", "2", "R1"])) & (oncokb["Gene"] != "Other Biomarkers")
    ]  # 1, 2 and R1 (top evidence)
    oncokb_select = oncokb_select.rename(columns={"Drugs (for therapeutic implications only)": "Drugs"})
    oncokb_select["Drugs"] = oncokb_select["Drugs"].apply(lambda x: x.split(", "))
    oncokb_select = oncokb_select.explode("Drugs")
    oncokb_select = oncokb_select.reset_index()

    for index, row in oncokb_select.iterrows():
        d = row["Drugs"]
        if re.search("\+", d):
            d = " + ".join(sorted(d.split(" + ")))
            oncokb_select.loc[index, ["Drugs"]] = d

    drug_gene = oncokb_select.drop_duplicates(subset=["Drugs", "Gene"])

    for index, row in drug_gene.iterrows():
        alteration_sen = []
        alteration_res = []
        oncokb_subset = oncokb_select.loc[
            (oncokb_select["Drugs"] == row["Drugs"]) & (oncokb_select["Gene"] == row["Gene"])
        ]

        for index2, row2 in oncokb_subset.iterrows():
            if re.search("excluding", row2["Alterations"]) != None:
                alt = [row2["Alterations"]]
            else:
                alt = row2["Alterations"].split(", ")

            if row2["Level"] in ["1", "2", "3A"]:
                alteration_sen = alteration_sen + alt
            else:
                alteration_res = alteration_res + alt

        if len(alteration_sen) > 0 and len(alteration_res) > 0:
            response = "sensitivity / resistance"
            variation = (
                "; ".join(sorted(list(set(alteration_sen)))) + " / " + "; ".join(sorted(list(set(alteration_res))))
            )
        elif len(alteration_sen) > 0:
            response = "sensitivity"
            variation = "; ".join(sorted(list(set(alteration_sen))))
        else:
            response = "resistance"
            variation = "; ".join(sorted(list(set(alteration_res))))

        output.write("\t".join([row["Drugs"], row["Gene"], variation, response, "OncoKB"]) + "\n")
    output.close()
            ...
            if re.search("Compendium_Cancer_Genes.tsv", i.filename):
                with z.open(i.filename) as f:
                    ...
                    for index, row in data.iterrows():
                        output.write(
                            "\t".join(
                                [row["SYMBOL"], row["COHORT"], row["CANCER_TYPE"], str(row["QVALUE_COMBINATION"])]
                            )
                            + "\n"
                        )
...
def process_depmap():
    ...
    skewness_min = pd.DataFrame(skewness)
    skewness_min["min"] = skewness[skewness > -0.5].min()

    skewness_min.to_csv(pro_dir + depmap_pro, index=True, sep="\t", header=False)
...
def process_SL():
    ...
    sel = inputf.loc[inputf["padj"] < 0.25].groupby("dependency")
    ...
    if not inputf.empty:
        ...
        for gene in gene_list:
            ...
            for mut in mut_list:
                ...
                transcript_list = (
                    inputf.loc[(inputf["Gene name"] == gene) & (inputf["GENOMIC_MUTATION_ID"] == mut)][
                        "HGVSC_Transcript"
                    ]
                    .unique()
                    .tolist()
                ...
                for trans in transcript_list:
                    FATHMM = [
                        ...
                        for x in inputf.loc[
                            (inputf["Gene name"] == gene)
                            & (inputf["GENOMIC_MUTATION_ID"] == mut)
                            & (inputf["HGVSC_Transcript"] == trans)
                        ]["FATHMM prediction"].tolist()
                    ][0]
                    HGVSc = inputf.loc[
                        (inputf["Gene name"] == gene)
                        & (inputf["GENOMIC_MUTATION_ID"] == mut)
                    ...
                    outputf.write(
                        "\t".join(
                            [
                                ":".join([gene, trans, HGVSc]),
                                mut,
                                FATHMM,
                                str(samples_gene),
                                str(samples_mut),
                                str(total_cosmic_rec),
                            ]
                        )
                        + "\n"
                    )
...
def create_cosmic_temp_files(idx):
    ...
    gene_list = ["^" + x + "_" for x in cleaned_genes_group] + ["^" + x + "\t" for x in cleaned_genes_group]
...
def process_cosmic():
    ...
    # some records have an empty ID (not counted)
    command = (
        "zcat "
        + dwn_dir
        + cosmic_file
        + " | cut -f "
        + ",".join(cols)
        + ' | awk -F"\t" \'($3 != "" && $4 == "38") || $1 == "Gene name" {print}\' | gzip > '
        + pro_dir
        + outputn
    )
    subprocess.call(command, shell=True)
    ...
    gfreq = open(pro_dir + "cosmic_gene_freq.tsv", "w")
    gfreq.write("\t".join(["Gene name", "Gene_freq", "Total"]) + "\n")
    gfreq.close()
    gfreq = open(pro_dir + "cosmic_gene_freq.tsv", "a")
    listdir = os.listdir(pro_dir + "temp/")
    for efile in listdir:
        inputf = pd.read_csv(pro_dir + "temp/" + efile, sep="\t", header=0, low_memory=False)
        inputf[["Gene name"]] = inputf["Gene name"].str.split("_", expand=True)[[0]]
        gene_list = inputf["Gene name"].unique().tolist()
        for gene in gene_list:
            samples_gene = len(inputf.loc[inputf["Gene name"] == gene]["ID_sample"].unique().tolist())

            gfreq.write("\t".join([gene, str(samples_gene), str(total_cosmic_rec)]) + "\n")
    gfreq.close()
    ...
    result_list_tqdm = []
    for result in tqdm(
        a_pool.imap(process_file, os.listdir(pro_dir + "temp/")), total=len(os.listdir(pro_dir + "temp/"))
    ):
        result_list_tqdm.append(result)
    ...
    with open(pro_dir + "COSMIC.tsv", "ab") as outfile:
        outfile.write(
            bytes("\t".join(["Cosmic_key", "cosmic_id", "FATHMM", "Gene_freq", "Mut_freq", "Total"]) + "\n", "utf-8")
        )
        for f in read_files:
                    ...
                    for Symbol in Measure.iter("Symbol"):
                        for ElementValue in Symbol.iter("ElementValue"):
                            if ElementValue.attrib["Type"] == "Preferred":
                                SYMBOL.append(ElementValue.text)
                ...
                for Trait in TraitSet:
                    for Name in Trait.iter("Name"):
                        for ElementValue in Name.iter("ElementValue"):
                            if ElementValue.attrib["Type"] == "Preferred":
                                TRAIT.append(ElementValue.text)
...
def process_clinvar():
    ...
    for l in inputf:
        if re.search(
            "xml|<ReleaseSet|</ReleaseSet|<ClinVarSet|</ClinVarSet|<ReferenceClinVarAssertion|</ReferenceClinVarAssertion|<ClinVarAccession|</ClinVarAccession|<MeasureSet|</MeasureSet|<Measure|</Measure|<SequenceLocation|</SequenceLocation|<Symbol|</Symbol|<ElementValue|</ElementValue|<TraitSet|</TraitSet|<Trait|</Trait|<Name|</Name|<ClinicalSignificance|</ClinicalSignificance|<Description|</Description",
            l.decode(),
        ):
            outputf.write(l)
    ...
    outputf.write(
        "\t".join(["Acc", "Assembly", "Chr", "VCF_pos", "VCF_ref", "VCF_alt", "Gene", "Trait", "Significance"]) + "\n"
    )
    outputf.close()
    ...
    for line in xml:
        ...
        if re.search("^<\?xml version|^<ReleaseSet|^</ReleaseSet", line):
            ...
            xml_section_file.write(
                '<ReleaseSet Dated="2021-10-30" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" Type="full"'
                ' xsi:noNamespaceSchemaLocation="http://ftp.ncbi.nlm.nih.gov/pub/clinvar/xsd_public/clinvar_public_1.64.xsd">'
                + "\n"
            )
...
def process_pfam():
    ...
    outputf.write(
        "\t".join(["DOMAIN_ID", "PFAM_ACC", "DOMAIN_DESCRIPT", "PROTEIN_NAME", "PROTEIN_ACC", "START", "END"]) + "\n"
    )
    ...
    for line in filei:
        ...
        if line_a[0] == "//":
            ...
            if line_a[1] == "ID":
                domain_id = " ".join(line_a[4:])
            if line_a[1] == "AC":
                pfam_acc = " ".join(line_a[4:])
            if line_a[1] == "DE":
                domain_des = " ".join(line_a[4:])
            if line_a[0] == "#=GS" and len(line_a) > 10:
                if "AC" in line_a:
                    ...
                    outputf.write(
                        "\t".join([domain_id, pfam_acc, domain_des, protein_name, protein_acc, start, end]) + "\n"
                    )
...
def process_interpro():
    ...
    inputf = gzip.open(interpro_file, "rb")
    outputn = interpro_file.replace(".xml.gz", ".xml.filtered.gz")
    outputf = gzip.open(outputn, "wb")
    for line in inputf:
...
def process_uniprot():
    ...
    for l in inputf:
        ...
        if re.search(
            (
                "\?xml|<uniprot|</uniprot|xmlns:|xsi:|<entry|</entry|<accession>|</accession>|<gene>|</gene>|<name"
                ' .*type="primary"|<organism>|<organism |</organism>|<name type="common"'
            ),
            l,
        ):
            if re.search("<uniprot", l):
                l = "<uniprot>\n"
...
def process_hallmarks():
    print("Processing Hallmarks data...")

    hallmarks_dwn = "hallmarks.xlsx"
    hallmarks_file = "hallmarks.tsv"

    filei = pd.read_excel(dwn_dir + hallmarks_dwn)
    filei["Genes_list"] = filei["Genes"].apply(lambda x: x.split(", "))
    filei = filei.explode("Genes_list")
    filei.to_csv(pro_dir + hallmarks_file, index=False, sep="\t", header=True)
...
# process_KEGG_ind()
# process_gene_names()
# correct manually NKX2.1 interpreted separately as NKX2 and 1
# process_civic()
# process_oncoKB()
# process_intogen()
# process_depmap()
